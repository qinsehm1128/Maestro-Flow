...
| 12 | [`12-maestro-brain-unified-design.md`](./12-maestro-brain-unified-design.md) | **maestro-brain 统一设计（去过度设计，对齐 maestro）** —— 并发双代理分析 brain vs maestro 设计哲学；删死代码 brain-review、合并派生层、删投机字段、修 runRecord 真实 bug(decide --commit)；命令瘦身 486→342；修 dashboard 构建 | `src/brain/`(8 模块)、`.claude/commands/maestro-brain.md`(v11)、`dashboard/.../install-utils.ts` |
