...
- `maestro brain review-plan --difficulty <d> [--self-reported] [--code-changed] [--critical] [--impl-cli <c>] [--clis a,b]` →
  **强制**评审 tier（invariant#7 的 L2 下限）+ reviewer≠implementer 隔离（invariant#4）。返回 `{tier, reviewCli, isolation}`。
...
引擎模块（确定性逻辑）：`brain-schema`（类型/阈值）、`brain-store`（原子 ledger）、`brain-derive`（cursor + mandatory/optional 视图）、
`stop-predicate`（终止谓词）、`brain-decide`（决策 + 收敛 caps）、`brain-review`（评审 tier 下限 + 隔离 + 裁决聚合——**brain 独有，ralph 无跨会话评审**）、
`brain-await`（事件驱动挂起，仅 Claude）、`cmd-brain`（CLI）。`deriveRouterSignals`（修 `_router.json` bug）在 `src/utils/state-schema.ts`（与其他派生器同处，coordinator 与 brain 共用，不互相耦合）。单测 `src/brain/__tests__/`（60 例）。
**评审的确定性决策**（tier 下限 / reviewer≠implementer / fail-closed 裁决）由 `brain-review` **代码强制**（它们曾酿成 R7 假绿真实 bug）；
**评审的 stage 编排**（派生哪些 agent、verify→challenge 散文）仍是**提示词所有**——与 maestro"评审=作者化 agent"一致。
...
**先调引擎拿强制决策**：`maestro brain review-plan --difficulty <d> [--self-reported] [--code-changed] [--critical] --impl-cli <c> --clis <list>`
→ 返回 `{tier, reviewCli, isolation}`（`brain-review` **代码强制** invariant#7 L2 下限 + invariant#4 reviewer≠implementer）。按返回的 tier 执行下列 stage、用返回的 `reviewCli`。
档位（`--review` 可经 review-plan 的 `--tier` 强制；**含码自报成功 → 下限 L2**）：
