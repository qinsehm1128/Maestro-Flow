...
<!-- v9 — two-layer realized: TS engine `src/brain/` (schema/store/derive/stop-predicate/decide/convergence) + this prompt FSM. Code-enforces what was prompt-only. _router.json latent bug fixed. Changelogs at bottom. -->
...
`stop-predicate`（mandatory/optional 终止）、`brain-decide`（决策+收敛）、`brain-derive`（cursor+快照）。单测 `src/brain/__tests__/`（37 例）。
