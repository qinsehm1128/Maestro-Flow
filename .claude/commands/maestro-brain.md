...
## A_INIT (S_INIT)
1. 解析 $ARGUMENTS（auto/-y/executor/review/max_rounds/intent）。`max_rounds` 缺省 30。
2. 建 `<workflow>/.brain/brain-{ts}/ledger.json`（{ts}=`YYYYMMDD-HHMMSS`；schema 见 <ledger_schema>）。
   **终止契约写进 ledger.stop_condition**（这才是真正的循环终止依据，不依赖 `/goal`）。
3. **A_EMIT_GOAL（可选、非阻塞）**：若**非 auto** 且有人在场，可输出一段 `/goal` 文本供用户粘贴（host 可选增强）。
   **`--auto -y` 时跳过**（无人可粘贴；`/goal` 是 host 命令、非 maestro 命令、不是终止依据）。
