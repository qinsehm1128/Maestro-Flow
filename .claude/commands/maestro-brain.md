...
<!-- v10 — A_AWAIT code-ified as event-driven SUSPEND (brain-await, Claude-only) + review orchestration code-ified Workflow-style (brain-review). Real-binary task test passed. Changelogs at bottom. -->
...
- `maestro brain decide --signal ok|result-problem|roadmap-problem:<issue>|unfixable-external [--json]` →
  跑 A_DECIDE 引擎（优先级互斥穷尽 + 收敛 caps：revises≥2 demote / stuck≥3 give-up / unfixable 快路），返回 decision。
- `maestro brain await <child-status.json> --kind ralph|odyssey [--timeout-min N]` → **挂起**（事件驱动 fs.watch，非忙轮询）
  直到子会话到终态（v8 真实字段）；exit 0=completed，1=硬信号（paused/failed/timeout/missing）。这是 A_AWAIT 的代码化。
- `maestro brain status` → 会话摘要。
引擎模块：`brain-schema`（类型/阈值）、`brain-store`（原子 ledger 读写）、`router-signals`（修 `_router.json` 潜伏 bug 的派生层）、
`stop-predicate`（mandatory/optional 终止）、`brain-decide`（决策+收敛）、`brain-derive`（cursor+快照）、
`brain-await`（事件驱动挂起，仅 Claude）、`brain-review`（Workflow 式分档评审编排 verify→review→challenge→collab→synthesize）。
单测 `src/brain/__tests__/`（63 例）。
**skill-only 模式**（无 maestro CLI）按本文档在上下文内推理这些逻辑（引擎是规范，提示词是回退）。
</engine>
...
- **安装隔离（修 R12-INFO）**：委派的 `npm/pip install` 等**依赖操作**会顺父目录污染宿主 `package.json`/lock。
...
2. ◇**自主决策**：推进 / 插入修复 / 修正 roadmap。
   - **崩溃/超时重试有界（修 R10-D2）**：崩溃/超时恢复用 `convergence.crash_retries[unit]` 计数（**独立于 stuck**），
...
```json
{
  ...
  "available_clis": ["claude","codex"], "autonomous": true, "await_timeout_min": 10, "poll_interval_s": 20, "max_polls": 30,
  "stop_condition": "all MANDATORY milestones completed; OPTIONAL completed-or-acknowledged-deferred",
  "stop_predicate": {
    "mandatory_all_completed": true,
    "optional_all_resolved": "每个 optional milestone: completed 或 (deferred 且 defer_reason 非空=acknowledged)",
  ...
  "convergence": { "stuck": {}, "revises": {}, "crash_retries": {} },
...
<changelog_v7>
针对健壮性战役 Wave D（R10 崩溃/超时-PASS / R11 矛盾-PASS / R12 停止正确性-PASS）修复——对抗暴露表达力与操作化缺口：
- [R12 HIGH] **stop_predicate 支持 optional milestone**：milestone 带 `mandatory|optional`；谓词改为
  `mandatory.every(completed) && optional.every(completed||ack-deferred) && 无 open defect`；加终态 `completed-with-optional-deferred`。
  解决"optional 单元无法表达 → 永不停 或 错误阻塞"，不再靠手动覆写。
- [R10-D1 MED] A_AWAIT 超时参数化：`poll_interval_s`/`max_polls`/`await_timeout_min` 具体值 + `--await-timeout`，绝不无界轮询。
- [R10-D2 MED] 崩溃/超时重试有界：`crash_retries[unit]` 上限 2 → 超限 defer+defect，不靠 max_rounds 兜底 spin 掉预算。
- [R10-D3 LOW] `crashed/error/killed` 列为显式硬信号终态失败，立即路由不等满超时。
- [R10-D4 LOW] 崩溃重试前 re-READ + 半成品当未受信（防 clobber）。
- [R12-INFO] A_DELIVER 安装隔离：委派 install 防污染宿主清单/lock，VERDICT 前检查并还原。
...
<changelog_v9>
**代码化（学习 ralph/odyssey 两层架构）+ 修 `_router.json` 潜伏 bug。** 把先前纯提示词的逻辑落成 `src/brain/` TS 引擎：
- 新模块：`brain-schema`/`brain-store`(原子)/`router-signals`/`stop-predicate`/`brain-decide`/`brain-derive`/`cmd-brain` + `commands/brain.ts` + cli.ts 注册。
- **代码强制**：mandatory/optional stop_predicate、A_DECIDE 优先级互斥穷尽、收敛 caps(revises/stuck/crash_retries)、cursor 派生——不再只靠提示词遵从。
- **修 bug**：`deriveRouterSignals` 计算 `_router.json` 读取却从未被算的 `latest_artifact_type/has_pending_plans/all_phases_executed/milestones_total`，
  接进 `graph-walker.buildInitialContext`——`_router.json` 不再对真实项目冷启动塌缩成 to_analyze。
- **TDD**：37 例 brain 单测全过（stop-predicate/decide/router-contract/derive/cmd-brain 端到端）；coordinator 176 例回归零破坏；tsc 我方文件 0 错误。
- CLI：`maestro brain init|derive|decide|status`，提示词 FSM 在 maestro-cli 模式下调用之（skill-only 回退到上下文推理）。
</changelog_v9>

<changelog_v10>
**A_AWAIT 挂起化 + 评审编排代码化（学 Claude Agent SDK / Workflow），并通过真实二进制任务测试。** 仅 Claude。
- `brain-await.ts`：A_AWAIT 从"轮询"改为**事件驱动挂起**（`fs.watch` 唤醒 + 安全兜底 + 硬超时），仿 Agent SDK `receive_response()`
  的"按事件阻塞、不忙等"；用 v8 真实终态字段；`classifyChildStatus` 对缺字段/running 一律判非终态（绝不假绿）。
- `brain-review.ts`：评审三明治按 Claude Workflow 编排成确定性计划——`selectTier`（invariant#7 的 L2 下限）、
  `selectReviewIsolation`（评审者≠实现者；Claude-only 用 distinct 实例）、`planReview`（verify→review→challenge→[collab]→synthesize，
  collab=parallel 屏障；单 CLI 自动降档不跳过评审）、`aggregateVerdict`（任一 false-green/gap/低置信 → 失败）。
- CLI 新增 `maestro brain await <status.json> --kind ralph|odyssey`。
- TDD：63 brain 单测全过（+await 12 +review 14）；tsc 我方文件 0 错误。
- **真实任务测试**：`npx tsc` emit → `node bin/maestro.js brain`：init/derive(cursor+mandatory/optional)/decide(advance) 正确；
  **await 真挂起**（子状态 1.5s 后翻 terminal，elapsedMs≈1432 被唤醒，exit 0）；optional 终止 → `completed-with-optional-deferred`。
</changelog_v10>
