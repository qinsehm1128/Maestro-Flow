...
4. **Pull in any prior critique** (optional signal): If `/maestro-impeccable critique` has been run on the same target, its priority issues are a useful prior for what to address first. Resolve the target to a file path or URL, then:
   ```bash
   slug=$(node .claude/skills/maestro-impeccable/scripts/critique-storage.mjs slug "<resolved>")
   node .claude/skills/maestro-impeccable/scripts/critique-storage.mjs latest "$slug"
   ```
